...
ap.add_argument("-a", "--arquivo", required = True,help = "Arquivo com lista de folds")
...
allSet=pickle.load( open( args["arquivo"], "rb" ) )
...
if(nomeArquivo==None):
    nomeArquivo='k_fold_resultado/k_fold_'+( "KNN" if tipoClassificador==1 else "SVM")+"_"+( "MoCr" if metodo==1 else "HiCo")
...
for imagensTreino,imagensTeste in allSet:
...
titulo=args["titulo"]
if(titulo==None):
    titulo="K-fold "+": "+( "Mo. de Cromaticidade" if metodo==1 else "Histograma Colorido")+"-> "+( "KNN" if tipoClassificador==1 else "SVM")
